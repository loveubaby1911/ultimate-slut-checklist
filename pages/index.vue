        ...
        <div class="masonry-container">
          <template v-for="category in myCategories">
            <template v-if="getTasks(category).length > 0">
              <div :class="['category', {'completed': (getCheckedTasksAmount(category) >= getTasks(category).length)}]" :key="category">
                <div class="title-container">
                  <h2>{{ category }}</h2>
                ...
                <div v-for="item in getTasks(category)" :key="item.name" :class="['task', {checked: checkedTasks.includes(item.name)}]">
